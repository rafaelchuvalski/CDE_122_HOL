...
def parseProperties():
    ...
    return maxParticipants, storageLocation, dataRows
...
def createTransactionData(spark, dataRows, ccMinValue):
    """
    ...
    try:
        ...
        dg = BankDataGen(spark, dataRows, ccMinValue)
...
def createTransactionBatch(spark, dataRows, ccMinValue):
    """
    Method to create a Banking Transactions dataframe using the dbldatagen and Faker frameworks
    """
...
def createSecondTransactionBatch(spark, dataRows, ccMinValue):
    """
    Method to create a Banking Transactions dataframe using the dbldatagen and Faker frameworks
    """
...
def createPiiData(spark, dataRows, ccMinValue):
    """
...
def main():
    ...
    for i in range(int(maxParticipants)):
        if i+1 < 10:
            username = "obs00" + str(i+1)
        elif i+1 > 9 and i+1 < 99:
            username = "obs0" + str(i+1)
        elif i+1 > 99:
            username = "obs" + str(i+1)

        print("PROCESSING USER {}...\n".format(username))

        bankTransactionsDf = createTransactionData(spark, dataRows, ccMinValue)
        saveTransactionData(bankTransactionsDf, storageLocation, username)

        piiDf = createPiiData(spark, dataRows, ccMinValue)
        savePiiData(piiDf, storageLocation, username)

        transactionsBatchDf = createTransactionBatch(spark, dataRows, ccMinValue)
        saveTransactionBatch(transactionsBatchDf, storageLocation, username)

        secondTransactionsBatchDf = createSecondTransactionBatch(spark, dataRows, ccMinValue)
